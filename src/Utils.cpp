...
namespace Utils
{
	...
	RE::NiAVObject* FindBoneNode(const RE::Actor* a_actorptr, const std::string_view a_nodeName, const bool a_isFirstPerson) {
		...
		while (!queue.empty()) {
			...
			if (RecursionCheck-- <= 0) {
				//MessageBoxA(NULL, "RECURSION BUG IN FindBoneNode()", "Precision", 0);
				queue.clear();
				return nullptr;
			}
			try {
					...
					if (const auto ninode = currentnode->AsNode()) {
						for (const auto& child : ninode->GetChildren()) {
							if (RecursionCheckFor-- <= 0) {
								//MessageBoxA(NULL, "RECURSION BUG IN FindBoneNode() For Loop", "Precision", 0);
								queue.clear();
								return nullptr;
							}
							// Bredth first search
				...
				//MessageBoxA(NULL, "Overflow Exception in FindBoneNode()", "Precision", 0);
				return nullptr;
			}  // this executes if f() throws std::overflow_error (same type rule)
			catch (const std::runtime_error& e) {
				SKSE::log::warn("Find Bone Underflow: {}", e.what());
				//MessageBoxA(NULL, "Underflow Exception in FindBoneNode()", "Precision", 0);
				return nullptr;
			}  // this executes if f() throws std::underflow_error (base class rule)
			catch (const std::exception& e) {
				SKSE::log::warn("Find Bone Exception: {}", e.what());
				//MessageBoxA(NULL, "STD Exception in FindBoneNode()", "Precision", 0);
				return nullptr;
			}  // this executes if f() throws std::logic_error (base class rule)
			catch (...) {
				SKSE::log::warn("Find Bone Exception Other");
				//MessageBoxA(NULL, "Other Exception in FindBoneNode()", "Precision", 0);
				return nullptr;
			}
		}
